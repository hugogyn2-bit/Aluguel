...
export default function TenantSignContractPage({ params }: { params: { id: string } }) {
  ...
  async function load() {
    ...
    setSignature(data.contract.tenantSignatureDataUrl || null);
  ...
  async function sign() {
    ...
    const res = await fetch(`/api/contracts/${contractId}/tenant-sign`, {
      method: "POST",
    ...
    setMsg("Contrato assinado pelo INQUILINO ✅");
    await load();
  }
  ...
  return (
    <div className="min-h-screen px-6 py-12 text-white">
      <div className="mx-auto max-w-4xl rounded-2xl border border-white/10 bg-white/5 p-8 backdrop-blur-xl">
        <h1 className="text-2xl font-extrabold">Assinar Contrato (INQUILINO)</h1>
        ...
        <div className="mt-6">
          <p className="text-sm text-white/70 mb-2">Assinatura do INQUILINO:</p>
